...
def home():
    ...
    math_count = db.session.query(terms).join(setList).filter(setList.category == 'Math', setList.author_id == current_user.id).count()
    math_sets = db.session.query(setList).filter(setList.category == 'Math', setList.author_id == current_user.id).count()
...
def create_set():
    ...
    for line in flashcards_data.splitlines():
        ...
        db.session.flush()
        userterms_instance = userTerms(user_id=current_user.id, term_id=terms_instance.id, card_json=Card().to_json(), set_id=set_instance.id)
